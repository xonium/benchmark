import { Button, Card, Col, Popover, Row, Space, Tag } from "antd";
...
export const Benchmark = observer(() => {
  ...
  const localStore = useLocalStore(() => ({
    showBenchmarkPopover: false,
    setShowBenchmarkPopover(value: boolean) {
      localStore.showBenchmarkPopover = value;
    },
  }));
    ...
    <Row gutter={BenchmarkGutter}>
      ...
      <Col span={24}>
        <Card
          ...
          extra={
            <Popover
              visible={localStore.showBenchmarkPopover}
              onVisibleChange={(visible) => {localStore.setShowBenchmarkPopover(visible)}}
              placement="left"
              content={
                <div style={{ maxWidth: "150px" }}>
                  <p>{t("benchmark_info_1")}</p>
                  <Button
                    icon={<SettingOutlined />}
                    block
                    onClick={() => {
                      localStore.setShowBenchmarkPopover(false);
                      UIStore.setShowSettingsDrawer(
                        !UIStore.showSettingsDrawer
                      );
                    }}
                  >
                    {t("settings")}
              ...
              trigger="click"
            >
              <InfoCircleOutlined onClick={() => localStore.setShowBenchmarkPopover(true)}/>
            </Popover>
          }
        >
          <Row gutter={BenchmarkGutter}>
      ...
      <Col span={24}>
        <Card
          title={t("calculator")}
          extra={
            <RouterLink routeName={RouteNames.StandaloneAmrapCalculator}>
              {t("standalone_calculator")}
            </RouterLink>
          }
        >
          <AmrapCalculator
            wodLengthInSeconds={
              (benchmarkStore.selected?.WorkoutDefinition.Minutes ?? 0) * 60
            }
            repsPerRound={
              benchmarkStore.selected?.WorkoutDefinition.RepsPerRound ?? 0
            }
            maxNumberOfRounds={
              benchmarkStore.selected?.WorkoutDefinition.MaxNumberOfRounds ?? 0
            }
